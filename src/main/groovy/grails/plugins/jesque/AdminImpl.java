...
public class AdminImpl implements Admin {
    ...
    public AdminImpl(final Config config, final Set<String> channels, final JobFactory jobFactory) {
        this(config, channels, jobFactory, new JedisPool(new GenericObjectPoolConfig(), config.getHost(), config.getPort(), config.getTimeout()));
    }
     ...
     */
    public AdminImpl(final Config config, final Set<String> channels, final JobFactory jobFactory, final Pool<Jedis> jedis) {
        if (config == null) {
        ...
        if (config.getPassword() != null) {
            this.jedis.getResource().auth(config.getPassword());
        }
    ...
    public void run() {
        if (this.state.compareAndSet(NEW, RUNNING)) {
            try {
                ...
                while (!isShutdown()) {
                    this.jedis.getResource().subscribe(this.jedisPubSub, createFullChannels());
                }
            } finally {
