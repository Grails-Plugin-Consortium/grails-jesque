...
public class WorkerImpl implements Worker {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(net.greghaines.jesque.worker.WorkerImpl.class);
    ...
    public void run() {
        if (this.state.compareAndSet(NEW, RUNNING)) {
            try {
                ...
                this.jedis.getResource().sadd(key(WORKERS), this.name);
                this.jedis.getResource().set(key(WORKER, this.name, STARTED), new SimpleDateFormat(DATE_FORMAT).format(new Date()));
                ...
                this.jedis.getResource().srem(key(WORKERS), this.name);
                this.jedis.getResource().del(key(WORKER, this.name), key(WORKER, this.name, STARTED), key(STAT, FAILED, this.name),
                        key(STAT, PROCESSED, this.name));
                this.jedis.getResource().quit();
    ...
    public void setQueues(final Collection<String> queues) {
        ...
        this.queueNames.addAll((queues == ALL_QUEUES) // Using object equality on purpose
                ? this.jedis.getResource().smembers(key(QUEUES)) // Like '*' in other clients
                : queues);
    }
    ...
    private void authenticateAndSelectDB() {
        if (this.config.getPassword() != null) {
            this.jedis.getResource().auth(this.config.getPassword());
        }
        this.jedis.getResource().select(this.config.getDatabase());
    }
    ...
    protected void checkPaused() throws IOException {
        if (this.paused.get()) {
            synchronized (this.paused) {
                if (this.paused.get()) {
                    this.jedis.getResource().set(key(WORKER, this.name), pauseMsg());
                }
                ...
                this.jedis.getResource().del(key(WORKER, this.name));
            }
        }
    }
    ...
    protected void process(final Job job, final String curQueue) {
        try {
            ...
            this.jedis.getResource().set(key(WORKER, this.name), statusMsg(curQueue, job));
    ...
    private void removeInFlight(final String curQueue) {
        if (SHUTDOWN_IMMEDIATE.equals(this.state.get())) {
            ...
            this.jedis.getResource().lpop(key(INFLIGHT, this.name, curQueue));
        }
    }
    ...
    protected String lpoplpush(final String from, final String to) {
        String result = null;
        while (JedisUtils.isRegularQueue(this.jedis.getResource(), from)) {
            this.jedis.getResource().watch(from);
            // Get the leftmost value of the 'from' list. If it does not exist, there is nothing to pop.
            String val = null;
            if (JedisUtils.isRegularQueue(this.jedis.getResource(), from)) {
                val = this.jedis.getResource().lindex(from, 0);
            }
            if (val == null) {
                this.jedis.getResource().unwatch();
                result = val;
                break;
            }
            final Transaction tx = this.jedis.getResource().multi();
            tx.lpop(from);
            tx.lpush(to, val);
            if (tx.exec() != null) {
                result = val;
