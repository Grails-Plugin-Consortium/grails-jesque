...
class JesqueSchedulerService {
    ...
    void enqueueJob(Jedis redis, String jobName, String hostName) {
        ...
        Class clazz = grailsApplication.getClassForName(scheduledJob.jesqueJobName)

        if (clazz) {
            def job = new Job(clazz.simpleName, scheduledJob.args)
    ...
    public void cleanUpStaleServers() {
        ...
        redisService.withRedis { Jedis redis ->
            ...
            staleServerHash = serverCheckInHash.findAll {
                Seconds.secondsBetween(new DateTime(it.value as Long), now).seconds > STALE_SERVER_SECONDS
            }
        }
    ...
    public void cleanUpStaleServer(String hostName) {
        redisService.withRedis { Jedis redis ->
            ...
            def triggerFireTime = staleJobNames.collect {
                [(it): redis.hget("$TRIGGER_PREFIX:$it", TriggerField.NextFireTime.name)]
            }.sum()
